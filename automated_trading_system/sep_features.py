...
def add_sep_features(sep_sampled, sep, sf1_art):
    ...
    sep_filled["mom6m"] = (sep_filled_1m_ago["adj_close"] / sep_filled_6m_ago[
        "adj_close"]) - 1  # 5-month cumulative returns ending one month before month end.
    sep_filled["mom12m"] = (sep_filled_1m_ago["adj_close"] / sep_filled_12m_ago[
        "adj_close"]) - 1  # 11-month cumulative returns ending one month before month end.
    ...
    for date, row in sep_sampled.iterrows():
        ...
        if date >= (first_date + relativedelta(years=1)):
            ...
            illiquidity_df["dollar_vol"] = (
                        ((sep_past_year["open"] + sep_past_year["close"]) / 2) * sep_past_year["volume"])
        ...
        if date >= (first_date + relativedelta(months=1)):
            ...
            sep_sampled.at[date, "std_turn"] = (sep_past_1month["volume"] / sep_past_1month[
                "sharesbas"]).std() * math.sqrt(22)
            ...
            if (number_of_trading_days_past_month != 0) and (monthly_turnover != 0):
                sep_sampled.at[date, "zerotrade"] = (num_zero_trading_days_the_past_month + (
                            1 / monthly_turnover) / deflator) * 21 / number_of_trading_days_past_month
...
if __name__ == "__main__":
    ...
    sep_sampled_df = sep_sampled_df.drop(
        sep_sampled_df.columns[sep_sampled_df.columns.str.contains('unnamed', case=False)], axis=1)
