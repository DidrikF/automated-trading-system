...
def setup():
    ...
    sf1_art_featured = pd.read_csv("./datasets/testing/sf1_art_featured_snapshot.csv",
...
def test_running_add_industry_sf1_features():
    ...
    industry_sf1_art_featured = pandas_mp_engine(callback=add_industry_sf1_features, atoms=sf1_art_featured, \
                                                 data={'metadata': metadata}, molecule_key='sf1_art',
...
def test_add_ms():
    ...
    if (art_row_cur["assetsavg"] != 0) and (art_row_cur["netinc"] != 0):
        i1_roa_above_avg = 1 if (
                    (art_row_cur["netinc"] / art_row_cur["assetsavg"]) > industry_mean_return_on_assets) else 0

        i2_cf_roa_above_avg = 1 if (
                    (art_row_cur["ncfo"] / art_row_cur["assetsavg"]) > industry_mean_cash_flow_return_on_assets) else 0
        ...
        i7_capex_indensity = 1 if (
                    (art_row_cur["capex"] / art_row_cur["assetsavg"]) > industry_mean_capex_intensity) else 0

        i8_advertising_intensity = 1 if (
                    (art_row_cur["sgna"] / art_row_cur["assetsavg"]) > industry_mean_advertising_intensity) else 0
