...
from automated_trading_system.sep_preparation import add_weekly_and_12m_stock_returns, \
    add_equally_weighted_weekly_market_returns, dividend_adjusting_prices_backwards, dividend_adjusting_prices_forwards
...
def test_add_equally_weighted_weekly_market_returns():
    ...
    assert sep.loc[sep.index == "2000-01-07", "mom1w"].mean() == pytest.approx(
        sep_aapl.loc["2000-01-07"]["mom1w_ewa_market"])
