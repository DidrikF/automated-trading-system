...
from automated_trading_system.sampling import extend_sep_for_sampling, first_filing_based_sampling, \
...
def setup():
    ...
    sf1_art = pd.read_csv("./datasets/testing/sf1_art.csv", parse_dates=["calendardate", "datekey"],
                          index_col="calendardate", low_memory=False)
    sf1_arq = pd.read_csv("./datasets/testing/sf1_arq.csv", parse_dates=["calendardate", "datekey"],
                          index_col="calendardate", low_memory=False)
...
def test_rebase_at_each_filing_sampling():
    ...
    sep_sampled = pandas_mp_engine(callback=rebase_at_each_filing_sampling, atoms=sep_extended, data=None, \
                                   molecule_key='observations', split_strategy='ticker', num_processes=1,
...
def test_first_filing_based_sampling():
    ...
    sep_extended = pandas_mp_engine(callback=first_filing_based_sampling, atoms=sep_extended, \
                                    data=None, molecule_key='sep', split_strategy='ticker', num_processes=1,
                                    molecules_per_process=1)
