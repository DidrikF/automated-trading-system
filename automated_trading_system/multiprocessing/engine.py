...
def pandas_mp_engine(callback, atoms, data, molecule_key, split_strategy, num_processes, molecules_per_process,
                     **kwargs):
    """
...
sf1_art_base = pd.read_csv("./datasets/sharadar/SF1_ART_BASE.csv", parse_dates=["calendardate", "datekey"],
                           index_col="calendardate")
sf1_arq_base = pd.read_csv("./datasets/sharadar/SF1_ARQ_BASE.csv", parse_dates=["calendardate", "datekey"],
                           index_col="calendardate")
        ...
        if task["data"] is not None:
            ...
            for kw_name, molecule_dict_name in task["data"].items():
                ...
                try:
                    ...
                    print("Keys of desired ({}) molecules_dict: {}".format(molecule_dict_name,
                                                                           molecules_dict[molecule_dict_name].keys()))
...
def split_df_into_molecules(atoms, split_strategy, num_molecules):
    ...
    if split_strategy == 'date':
        ...
        for i in range(1, len(parts)):
            ...
            report_progress(i, len(parts), time0,
                            "split_df_into_molecules according to split strategy: " + split_strategy)
        ...
        raise ValueError(
            "split_strategy cannot be " + split_strategy + ". Only 'ticker', 'industry' and 'date' are supported.")
...
def pandas_chaining_mp_engine(tasks, primary_atoms, atoms_configs, split_strategy, num_processes, cache_dir, \
        ...
        for i_file in task_cache_files:
            ...
            if file_name in files_in_cache:
                ...
                if task_we_are_loading_output_from["split_strategy"] != new_first_task["split_strategy"]:
                    print(
                        "Resplitting primary_molecules when resuming because they were cached in the wrong format, needed: ",
                        new_first_task["split_strategy"])
                    atoms = combine_molecules(primary_molecules)
                    primary_molecules = split_df_into_molecules(atoms, new_first_task["split_strategy"],
                                                                num_processes * molecules_per_process)
        ...
        if os.path.isfile(pickle_path):
            ...
            if (split_strategy_changed == True) and (tasks[0]["data"] is not None):
                ...
                for molecules_dict_name in new_first_task["data"].values():
                    ...
                    molecules_dict[molecules_dict_name] = split_df_into_molecules(atoms,
                                                                                  new_first_task["split_strategy"],
                                                                                  num_processes * molecules_per_process)
        else:
            print("Reading and parsing: ", atoms_config["csv_path"])
            atoms = pd.read_csv(atoms_config["csv_path"], parse_dates=atoms_config["parse_dates"],
                                index_col=atoms_config["index_col"], low_memory=False)
            if atoms_config["sort_by"] is not None:
                atoms = atoms.sort_values(by=atoms_config["sort_by"])
            molecules_dict[disk_name] = split_df_into_molecules(atoms, split_strategy,
                                                                num_processes * molecules_per_process)  # Split strategy here might have changed

            if (atoms_config["cache"]) == True and (
                    resume == False):  # Only cache parsed atoms, when we are not resuming (which could have changed the split_strategy, making the contents of the cache harder to reason about)
    ...
    for index, task in enumerate(tasks):
        ...
        print("Step ", str(index + 1), " of ", str(len(tasks)), " - " + task["name"] + " - Time elapsed: ",
              str(round(minutes_elapsed, 2)), " minutes.")
        ...
        primary_molecules = process_jobs_fast(jobs, num_processes=num_processes,
                                              sort_by=sort_by)  # return as list of Data Frames, I need a dict
        ...
        if task["cache_result"] == True:
            ...
            if (task.get("add_to_molecules_dict", False) == True) and (
                    task["split_strategy"] != task["split_strategy_for_molecule_dict"]):
                ...
                atoms = combine_molecules(primary_molecules)
                molecules_to_cache = split_df_into_molecules(atoms, task["split_strategy_for_molecule_dict"],
                                                             num_processes * molecules_per_process)
        ...
        if index < last_index:
            ...
            if next_task["split_strategy"] != task["split_strategy"]:
                atoms = combine_molecules(primary_molecules)
                primary_molecules = split_df_into_molecules(atoms, next_task["split_strategy"],
                                                            num_processes * molecules_per_process)
        ...
        if index < last_index:
            ...
            if next_task["data"] is not None:
                for molecules_dict_name in next_task["data"].values():
                    print(
                        "Splitting data molecules just in case the require split strategy is different from the current one.")
                    atoms = combine_molecules(molecules_dict[molecules_dict_name])
                    molecules_dict[molecules_dict_name] = split_df_into_molecules(atoms, next_task["split_strategy"],
                                                                                  num_processes * molecules_per_process)
        ...
        if task.get("add_to_molecules_dict", False) == True:
            ...
            if task["split_strategy_for_molecule_dict"] != next_task["split_strategy"]:
                atoms = combine_molecules(primary_molecules)
                molecules_to_add_to_molecules_dict = split_df_into_molecules(atoms,
                                                                             task["split_strategy_for_molecule_dict"],
                                                                             num_processes * molecules_per_process)
