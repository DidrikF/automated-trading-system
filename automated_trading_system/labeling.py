...
def getEvents(close: pd.Series, tEvents: pd.DatetimeIndex, ptSl: tuple, trgt: pd.Series, minRet: float, numThreads: int,
              t1=False):
    ...
    df0 = pandas_mp_engine(func=apply_ptsl_on_t1, pdObj=('molecule', events.index), numThreads=numThreads, \
                           ...
                           ptSl=[ptSl, ptSl])  # notice that barriers are symmetric when first labeling for side.
    ...
    events["t1"] = df0.dropna(how='all').min(
        axis=1)  # pd.min ignores nan, here events["t1"] becomes the timesamp of the earliest barrier touch
    events = events.drop("side", axis=1)

    return events


def apply_ptsl_on_t1(close, events, ptSl,
                     molecule):  # Need to rewrite, molecule is not needed at the splitting will be done prior to this function being called
    """
    ...
    for loc, t1 in events_["t1"].fillna(close.index[-1]).iterrows():  # path prices (iteritems())
        ...
        df0 = (df0 / close[loc] - 1) * events_.at[
            loc, "side"]  # path returns (this takes into account both long and short positions)
...
def triple_barrier_search(events, sep, ptSl):
    ...
    take_profit_barriers = ptSl[0] * events[
        "ewmstd"]  # I only consider the case where there is both horizontal barriers
    stop_loss_barriers = ptSl[1] * events["ewmstd"]

    for date, timeout in events["timeout"].fillna(sep.index[
                                                      -1]).iterrows():  # If timeout is missing for an event (this should not be the case), we use the last date in sep (for the ticker) as timeout.
        path_of_prices = sep[date:timeout]["close"]
        path_of_returns = (path_of_prices / path_of_prices[0] - 1) * events.at[
            date, "side"]  # remember we are still allways long
        barrier_touch_dates.loc[date, "earliest_take_profit_touch"] = path_of_returns[
            path_of_returns > take_profit_barriers[date]].index.min()
        barrier_touch_dates.loc[date, "earliest_stop_loss_touch"] = path_of_returns[
            path_of_returns < stop_loss_barriers[date]].index.min()
...
def get_first_barrier_touches_complete(sep_featured: pd.DataFrame, sep: pd.DataFrame, ptSl: tuple, min_ret):
    ...
    # 3) form events objects, apply stop loss on t1
    events = pd.concat(
        {'timeout': timeout["timeout"], 'ewmstd': ewmstd["ewmstd_2y_monthly"], 'side': side_long["side"]},
    ...
    for date, timeout in events["timeout"].fillna(sep.index[
                                                      -1]).iteritems():  # If timeout is missing for an event (this should not be the case), we use the last date in sep (for the ticker) as timeout.
        path_of_prices = sep.loc[(sep.index >= date) & (sep.index <= timeout)][["close"]]
        path_of_returns = ((path_of_prices["close"] / path_of_prices.iloc[0]["close"]) - 1) * \
                          events.loc[events.index == date].iloc[0][
                              "side"]  # events.loc[date]["side"] # remember we are still allways long
        barrier_touch_dates.loc[date, "earliest_take_profit_touch"] = path_of_returns[
            path_of_returns > take_profit_barriers[date]].index.min()
        barrier_touch_dates.loc[date, "earliest_stop_loss_touch"] = path_of_returns[
            path_of_returns < stop_loss_barriers[date]].index.min()
    ...
    # print(events)
    # drop those where none of the barrieres where touched (should be extremly rare, if not at all I think)
    events["earliest_touch"] = barrier_touch_dates.dropna(how='all').min(
        axis=1)  # pd.min ignores nan, here events["t1"] becomes the timesamp of the earliest barrier touch
...
if __name__ == "__main__":
    ...
    sep_featured = pd.read_csv("./datasets/ml_ready_live/sep_featured_done.csv",
                               parse_dates=["date", "datekey", "timeout"])
