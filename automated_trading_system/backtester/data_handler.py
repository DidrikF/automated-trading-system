...
class DailyBarsDataHander(DataHandler):
    def __init__(self, source_path, store_path: str, file_name_time_data, file_name_ticker_data, start: pd.datetime,
                 end: pd.datetime):
    ...
    def next_tick_old(self):
        ...
        while True:
            if self.cur_date > self.end:
                ...
                raise IndexError(
                    "The current date is beyond the end date of the backtest. DataHandler cannot return next_tick.")
...
class MLFeaturesDataHandler(DataHandler):
    ...
    def current(self, current_date) -> Event:
        ...
        return Event(event_type="FEATURE_DATA", data=pd.DataFrame(),
                     date=current_date)  # Will contain feature data for last business day before current_date
