...
from automated_trading_system.backtester.data_handler import DataHandler
from automated_trading_system.backtester.event import EventQueue
from automated_trading_system.backtester.portfolio import Portfolio
from automated_trading_system.backtester.utils import report_progress
...
class Backtester():
    def __init__(self, market_data_handler: DataHandler, feature_data_handler: DataHandler, start, end, output_path,
        ...
        handle_data_hook :  Called every tick of the backtest. Can be used to calculate and track data to add to perf
                            data frame.
        ...
        # OBS:  Not used atm, might just drop these and store all in perf. And whatever that
        #       might had gone into context, is handled by the portfolio and strategy.
    ...
    def set_portfolio(self, portfolio_cls, **kwargs):
        ...
        - Can I assume dividend reinvestment plans are available for all stocks? Even if it only is a service provided
          by the broker?
    ...
    def set_broker(self, broker_cls, **kwargs):
        """
        Set broker instance that will execute orders. Orders come from a portfolio object and contain information like:
        ticker, amount, side, stop-loss and take-profit.

        The broker fills only orders with amount under 10% of the previous day's volume. If the order exceeds this
        limit, the order will be broken up. This is to ensure that slippage does not become a major factor.

        When filling an order the broker calculates commission and slippage (costs) and charges the portfolio
        accordingly. Orders are filled at the opening price (plus slippage). Multiple consecutive orders are filled so
        fast that any random fluctuation the market may have exhibited over the few seconds (or even milliseconds)
    ...
    def run(self):
        ...
        while True:  # This loop generates new "ticks" until the backtest is completed.
            ...
            while True:
                try:
                    ...
                    if event.type == 'DAILY_MARKET_DATA':
                        # need this to add the feature data to the queue,
                        # so it can be "processed" (used to generate predictions)
                        feature_data_event = self.feature_data.current(
                            event.date)  # the function need access to the event queue

                        if feature_data_event is not None:
                            self._event_queue.add(
                                feature_data_event)  # The next iteration of the loop, the event queue is not empty.
                        ...
                        # Might get no fills or cancelled orders
                        ...
                        # Here the portfolios state with regards to active positions
                        # and return calculation can be handled

                        # Don't know what this will do yet. Dont know what it will return
                    ...
                    # Possible that these events are handled outside this loop
                        ...
                        # This happens at the end of the trading day. Funds that become available as the result of a
                        # position being liquidated is first available to use the next day.
                        ...
                        # dividend is one type of corporate action, if this is the only
                        # event I end up covering are rename this stuff
                        self.broker.handle_corporate_actions(event)
                        self.portfolio.handle_corporate_actions(event)

            # Here the day is over, before ending the day and starting a new one we want to update the margin account
            # according to latest close prices and update the balance and positions if any was liquidated throughout
            # the day.
            position_liquidations_event, margin_account_update_event = self.broker.manage_active_positions(
                self.portfolio)  # This generates margin account update
            ...
            if margin_account_update_event is not None:
                # Now the money from liquidated positions are available to update the margin account
            ...
            # Notice that this is called after margin_account has been updated and liquidation events have updated the
            # balance and the portfolio state
    ...
    def capture_daily_state(self):
        """
        Calculate various backtest statistics. These calculations are split into their own function, but the work is
        sentralized here.
        """
    ...
    def save_state_to_disk_and_return(self):
        """
        This will save a snapshot of all relevant state of the backtest and save it to disk as a pickle. This way the
        backtest can be replayed and all results are available for later inspection.
        ...
        - portfolio signlas         ->  all signals generated -> with reference to feature data, so it can be retrieved
        - portfolio order history   ->  all orders generated and send to the broker -> can be used with the blotter to
                                        see what orders was not filled
        ...
        backtest_state = {
            ...
            "broker": {
                ...
                "active_positions_history": self.broker.active_positions_history_to_df(),
                # this is the fills that are active at any one time, the fills combines makes up the portfolio,
                # this is somewhat different from portfolio.portfolio
