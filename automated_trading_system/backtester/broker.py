...
from automated_trading_system.backtester.errors import OrderProcessingError, MarketDataNotAvailableError, \
    BalanceTooLowError
from automated_trading_system.backtester.event import Event
from automated_trading_system.backtester.logger import Logger
from automated_trading_system.backtester.portfolio import CancelledOrder, Portfolio
from automated_trading_system.backtester.utils import CommissionModel, SlippageModel
...
class Broker:
    ...
    def process_orders(self, portfolio, orders_event):
        ...
        for order in orders:
            try:
                ...
                Logger.log.warning("Failed processing order with error: {}".format(e))
    ...
    def _process_order(self, portfolio, order):
        ...
        if len(validation_errors) != 0:
            Logger.log.warning("Order failed validation with validation errors: {}".format(validation_errors))
        ...
        try:
            ...
            Logger.log.warning(
                "When processing an order; market data was not available for ticker {} on date {}.".format(order.ticker,
                                                                                                           self.market_data.cur_date))
        ...
        if order.direction == 1:
            ...
            commission = self.commission_model.calculate(order,
                                                         fill_price)  # The commission is also based on the fill_price
            ...
            try:
                ...
                Logger.log.warning(
                    "Balance too low! Balance: {}, wanted to charge {} for the stock and {} in commission".format(
                        portfolio.balance, cost, commission))
                raise OrderProcessingError("Cannot complete order, with error: {}".format(e))
    ...
    def manage_active_positions(self, portfolio: Portfolio):
            ...
            if order.direction == 1:
                ...
                if price_direction_for_the_day == 1:
                    ...
                    if ticker_data["low"] <= order.stop_loss:
                        ...
                        commission = self.commission_model.calculate(order=order,
                                                                     fill_price=order.stop_loss)  # Do I need to change the signature?
                        portfolio.charge_commission(commission)
                        ...
                        # Calculate and charge commission
                        commission = self.commission_model.calculate(order=order,
                                                                     fill_price=order.take_profit)  # Do I need to change the signature?
                        portfolio.charge_commission(commission)
                        # Sell out of position
                    ...
                    if ticker_data["high"] >= order.take_profit:
                        ...
                        liquidated_positions.append(
                            PositionLiquidation(fill, order.take_profit, self.market_data.cur_date))
                        # pop out fill
                        self.active_positions.remove(fill)
                        continue
                ...
                if self.market_data.cur_date >= order.time_out:
                    # Calculate and charge commission
                    commission = self.commission_model.calculate(order=order, fill_price=ticker_data[
                        "close"])  # Do I need to change the signature?
                    portfolio.charge_commission(commission)
                ...
                if price_direction_for_the_day == 1:
                    # Prices rose over the day, low was hit first
                    if ticker["low"] <= order.take_profit:
                        # Calculate and charge commission
                        commission = self.commission_model.calculate(order=order,
                                                                     fill_price=order.take_profit)  # Do I need to change the signature?
                        portfolio.charge_commission(commission)
                        ...
                        "high"] >= order.stop_loss:  # For short positions, stop loss wil be higher than the fill price
                        # Calculate and charge commission
                        commission = self.commission_model.calculate(order=order,
                                                                     fill_price=order.stop_loss)  # Do I need to change the signature?
                        portfolio.charge_commission(commission)
                    ...
                    if ticker[
                        ...
                        self.active_positions.remove(fill)
                        continue
                    elif ticker["low"] <= order.take_profit:
                        # Calculate and charge commission
                        commission = self.commission_model.calculate(order=order,
                                                                     fill_price=order.take_profit)  # Do I need to change the signature?
                        portfolio.charge_commission(commission)
        ...
        if len(liquidated_positions) > 0:
            position_liquidations_event = Event(event_type="POSITION_LIQUIDATIONS", data=liquidated_positions,
                                                date=self.market_data.cur_date)
        else:
            position_liquidations_event = None

        margin_account_update_event = Event(event_type="MARGIN_ACCOUNT_UPDATE", data=required_margin_account_size,
                                            date=self.market_data.cur_date)
    ...
    def calculate_initial_margin_requirement(self, order):
        cur_price = self.get_most_updated_close_for_ticker(
            order.ticker)  # the price should allways be available at this point...
        if cur_price is None:
            raise MarketDataNotAvailableError(
                "Failed to calculate initial margin requirement, because data not avialable for ticker {} on {}".format(
                    order.ticker, self.market_data.cur_date))

        short_position_value = cur_price * order.amount
