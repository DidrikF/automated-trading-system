...
from automated_trading_system.backtester.errors import MarketDataNotAvailableError
...
from automated_trading_system.backtester.portfolio import Order
...
class RandomLongShortStrategy:
    ...
    def generate_orders_from_signals(self, portfolio, signals_event: Event):
        ...
        for signal in signals:
            ...
            orders.append(
                Order(order_id=self.get_order_id(), ticker=ticker, amount=amount, date=self.market_data.cur_date,
                      signal=signal, \
    ...
    def set_order_restrictions(self, max_size, max_positions, min_positions, max_orders_per_day, max_orders_per_month,
                               max_hold_period):
        """
...
class BuyAppleStrategy():
    ...
    def generate_signals(self, feature_data_event: Event):
        ...
        signals.append(
            Signal(signal_id=self.get_signal_id(), ticker="AAPL", direction=1, certainty=1, ewmstd=None, ptSl=None))
