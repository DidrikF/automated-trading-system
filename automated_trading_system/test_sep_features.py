...
from automated_trading_system.helpers.helpers import get_x_past_months_of_data
...
def test_add_return():
    ...
    sep_featured = pandas_mp_engine(callback=add_sep_features, atoms=sep_sampled, \
                                    data={'sep': sep_prepared, "sf1_art": sf1_art}, molecule_key='sep_sampled',
...
def test_add_mom1m_mom6m_mom12m_mom24m():
    ...
    assert sep_featured_aapl.loc["2001-04-12"]["mom6m"] == (
                sep_prepared_aapl.loc["2001-03-13"]["adj_close"] / sep_prepared_aapl.loc["2000-10-12"]["adj_close"]) - 1
...
def test_add_chmom():
    ...
    chmom_at_2001_04_12 = (sep_prepared_aapl.loc["2001-03-13"]["adj_close"] / sep_prepared_aapl.loc["2000-10-12"][
        "adj_close"] - 1) - \
                          (sep_prepared_aapl.loc["2000-09-12"]["adj_close"] / sep_prepared_aapl.loc["2000-04-12"][
                              "adj_close"] - 1)

    assert sep_featured.loc[(sep_featured.ticker == "AAPL") & (sep_featured.index == "2001-04-12")].iloc[0][
               "chmom"] == chmom_at_2001_04_12
...
def test_add_illiquidity():
    ...
    assert sep_featured.loc[(sep_featured.ticker == "AAPL") & (sep_featured.index == "1999-04-08")].iloc[0][
               "ill"] == pytest.approx(-1.5139424798827458e-11)
...
def test_add_std_dolvol():
    ...
    assert sep_featured.loc[(sep_featured.ticker == "AAPL") & (sep_featured.index == "1998-02-09")].iloc[0][
               "std_dolvol"] == \
           std_dolvol
