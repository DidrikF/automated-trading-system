...
def add_industry_sf1_features(sf1_art, metadata):
    ...
    for index_cur, art_row_cur in sf1_art.iterrows():
        ...
        sf1_art_for_date = sf1_art_for_date.drop_duplicates(subset="ticker",
                                                            keep="first")  # Keeping the first gets out of any lookahead bias
        ...
        if caldate_cur not in industry_means.index:
            ...
            industry_mean_bm = (sf1_art_for_date["equityusd"] / sf1_art_for_date[
                "marketcap"]).mean()  # I think it automatically excludes rows which leads to zero in the denominator.
            industry_mean_cfp = (sf1_art_for_date["ncfo"] / sf1_art_for_date[
                "marketcap"]).mean()  # I think it automatically excludes rows which leads to zero in the denominator.
            industry_mean_asset_turnover = (sf1_art_for_date["change_sales"] / sf1_art_for_date[
                "assetsavg"]).mean()  # I think it automatically excludes rows which leads to zero in the denominator.
            ...
            industry_means.at[
                caldate_cur, "industry_mean_cash_flow_return_on_assets"] = industry_mean_cash_flow_return_on_assets
        ...
        if art_row_cur["marketcap"] != 0:
            sf1_art.at[index_cur, "bm_ia"] = (art_row_cur["equityusd"] / art_row_cur["marketcap"]) - industry_means.at[
                caldate_cur, "industry_mean_bm"]
        ...
        if art_row_cur["marketcap"] != 0:
            sf1_art.at[index_cur, "cfp_ia"] = (art_row_cur["ncfo"] / art_row_cur["marketcap"]) - industry_means.at[
                caldate_cur, "industry_mean_cfp"]
        ...
        if art_row_cur["assetsavg"] != 0:
            sf1_art.at[index_cur, "chatoia"] = (art_row_cur["change_sales"] / art_row_cur["assetsavg"]) - \
                                               industry_means.at[caldate_cur, "industry_mean_asset_turnover"]
        ...
        if art_row_cur["marketcap"] != 0:
            sf1_art.at[index_cur, "mve_ia"] = art_row_cur["marketcap"] - industry_means.at[
                caldate_cur, "industry_mean_marketcap"]
        ...
        sf1_art.at[index_cur, "pchcapex_ia"] = art_row_cur["grcapx"] - industry_means.at[
            caldate_cur, "industry_mean_percent_change_capex"]
        ...
        sf1_art.at[index_cur, "chpmia"] = art_row_cur["chprofitmargin"] - industry_means.at[
            caldate_cur, "industry_mean_change_profit_margin"]
...
def get_ms(art_row_cur: pd.Series, industry_means: pd.DataFrame, caldate_cur: pd.datetime) -> int:
    ...
    if (art_row_cur["assetsavg"] != 0) and (art_row_cur["netinc"] != 0):
        i1_roa_above_avg = 1 if ((art_row_cur["netinc"] / art_row_cur["assetsavg"]) > industry_means.at[
            caldate_cur, "industry_mean_return_on_assets"]) else 0

        i2_cf_roa_above_avg = 1 if ((art_row_cur["ncfo"] / art_row_cur["assetsavg"]) > industry_means.at[
            caldate_cur, "industry_mean_cash_flow_return_on_assets"]) else 0

        i3_ncfo_exceeds_netinc = 1 if (art_row_cur["ncfo"] > art_row_cur["netinc"]) else 0

        i6_rnd_intensity = 1 if ((art_row_cur["rnd"] / art_row_cur["assetsavg"]) > industry_means.at[
            caldate_cur, "industry_mean_rnd_intensity"]) else 0

        i7_capex_indensity = 1 if ((art_row_cur["capex"] / art_row_cur["assetsavg"]) > industry_means.at[
            caldate_cur, "industry_mean_capex_intensity"]) else 0

        i8_advertising_intensity = 1 if ((art_row_cur["sgna"] / art_row_cur["assetsavg"]) > industry_means.at[
            caldate_cur, "industry_mean_advertising_intensity"]) else 0
...
if __name__ == "__main__":
    ...
    sf1_art = pd.read_csv("./datasets/testing/sf1_art.csv", index_col="datekey",
                          parse_dates=["datekey", "calendardate"])
    metadata = pd.read_csv("./datasets/sharadar/SHARADAR_TICKERS_METADATA.csv", index_col="ticker",
