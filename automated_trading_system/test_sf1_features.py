...
from automated_trading_system.helpers.helpers import forward_fill_gaps, get_most_up_to_date_10q_filing, \
    get_most_up_to_date_10k_filing
from automated_trading_system.multiprocessing.engine import pandas_mp_engine
from automated_trading_system.sf1_features import add_sf1_features
...
features = ["roaq", "chtx", "rsup", "sue", "cinvest", "nincr", "roavol", "cashpr", "cash", "bm", "currat", "depr", "ep",
            "lev", "quick", \
            "rd_sale", "roic", "salecash", "saleinv", "salerec", "sp", "tb", "sin", "tang", "debtc_sale",
            "eqt_marketcap", "dep_ppne", "tangibles_marketcap", \
            "agr", "cashdebt", "chcsho", "chinv", "egr", "gma", "invest", "lgr", "operprof", "pchcurrat", "pchdepr",
            "pchgm_pchsale", "pchquick", "pchsale_pchinvt", \
            "pchsale_pchrect", "pchsale_pchxsga", "pchsaleinv", "rd", "roeq", "sgr", "grcapx", "chtl_lagat",
            "chlt_laginvcap", "chlct_lagat", "chint_lagat", \
            "chinvt_lagsale", "chint_lagsgna", "chltc_laginvcap", "chint_laglt", "chdebtnc_lagat", "chinvt_lagcor",
            "chppne_laglt", "chpay_lagact", \
            "chint_laginvcap", "chinvt_lagact", "pchppne", "pchlt", "pchint", "chdebtnc_ppne", "chdebtc_sale", "age",
            "ipo", "profitmargin", \
...
def setup():
    ...
    sf1_art = pd.read_csv("./datasets/testing/lacking_sf1_art.csv",
                          parse_dates=["calendardate", "datekey", "reportperiod"], \
                          index_col="calendardate", low_memory=False)
    sf1_arq = pd.read_csv("./datasets/testing/lacking_sf1_arq.csv",
                          parse_dates=["calendardate", "datekey", "reportperiod"], \
                          index_col="calendardate", low_memory=False)
...
def test_running_add_sf1_features():
    ...
    sf1_art_featured.to_csv(
        "./datasets/testing/sf1_art_featured_snapshot.csv")  # Need to work some more with the testing and then I remove this line to not compremise the correct snapshot...
...
def test_setup_aapl_2001_03_31():
    ...
    art_row_cur = sf1_art_aapl_done.loc[sf1_art_aapl_done.calendardate == "2001-03-31"].iloc[
        -1]  # You can swap out this date, and it should work
...
def test_add_sf1_features_nincr():
    ...
    arq_rows = [arq_row_cur, arq_row_1q_ago, arq_row_2q_ago, arq_row_3q_ago, arq_row_4q_ago, arq_row_5q_ago,
                arq_row_6q_ago, arq_row_7q_ago]
...
def test_add_sf1_features_roic():
    ...
    roic = (art_row_cur["ebit"] - nopic_t_1) / (
...
def test_add_sf1_features_cashdebt():
    ...
    cashdebt = (art_row_cur["revenueusd"] + art_row_cur["depamor"]) / (
...
def test_add_sf1_features_invest():
    ...
    invest = ((art_row_cur["ppnenet"] - art_row_1y_ago["ppnenet"]) + (
...
def test_add_sf1_features_operprof():
    ...
    operprof = (art_row_cur["revenueusd"] - art_row_cur["cor"] - art_row_cur["sgna"] - art_row_cur["intexp"]) / \
               art_row_1y_ago["equityusd"]
...
def test_add_sf1_features_pchcurrat():
    ...
    pchcurrat = ((art_row_cur["assetsc"] / art_row_cur["liabilitiesc"]) / (
...
def test_add_sf1_features_pchdepr():
    ...
    pchdepr = ((art_row_cur["depamor"] / art_row_cur["ppnenet"]) / (
...
def test_add_sf1_features_pchgm_pchsale():
    ...
    pchgm_pchsale = ((gross_margin_t_1 / gross_margin_t_2) - 1) - (
...
def test_add_sf1_features_pchsale_pchinvt():
    ...
    pchsale_pchinvt = ((art_row_cur["revenueusd"] / art_row_1y_ago["revenueusd"]) - 1) - (
            (art_row_cur["inventory"] / art_row_1y_ago["inventory"]) - 1)
...
def test_add_sf1_features_pchsale_pchrect():
    ...
    pchsale_pchrect = ((art_row_cur["revenueusd"] / art_row_1y_ago["revenueusd"]) - 1) - (
            (art_row_cur["receivables"] / art_row_1y_ago["receivables"]) - 1)
...
def test_add_sf1_features_pchsale_pchxsga():
    ...
    pchsale_pchxsga = ((art_row_cur["revenueusd"] / art_row_1y_ago["revenueusd"]) - 1) - (
            (art_row_cur["sgna"] / art_row_1y_ago["sgna"]) - 1)
...
def test_add_sf1_features_pchsaleinv():
    ...
    pchsaleinv = ((art_row_cur["revenueusd"] / art_row_cur["inventory"]) / (
...
def test_add_sf1_features_chprofitmargin():
    ...
    chprofitmargin = (art_row_cur["netinc"] - art_row_cur["revenueusd"]) - (
