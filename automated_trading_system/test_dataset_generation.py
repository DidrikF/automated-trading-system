...
from automated_trading_system.sampling import extend_sep_for_sampling, rebase_at_each_filing_sampling
from automated_trading_system.sep_preparation import dividend_adjusting_prices_backwards, \
    add_weekly_and_12m_stock_returns, add_equally_weighted_weekly_market_returns
from automated_trading_system.sep_industry_features import add_indmom
from automated_trading_system.sep_features import add_sep_features

from automated_trading_system.sf1_features import add_sf1_features
from automated_trading_system.sf1_industry_features import add_industry_sf1_features
...
def test_sep_featured():
    ...
    sep_adjusted_plus_returns = pandas_mp_engine(callback=add_weekly_and_12m_stock_returns, atoms=sep_adjusted,
                                                 data=None, \
                                                 molecule_key='sep', split_strategy='ticker', \
                                                 num_processes=num_processes, molecules_per_process=1)

    sep_prepared = pandas_mp_engine(callback=add_equally_weighted_weekly_market_returns,
                                    atoms=sep_adjusted_plus_returns, data=None, \
    ...
    sep_sampled = pandas_mp_engine(callback=rebase_at_each_filing_sampling, atoms=sep_prepared_plus_indmom, data=None, \
                                   molecule_key='observations', split_strategy='ticker', num_processes=num_processes,
    ...
    sep_featured = pandas_mp_engine(callback=add_sep_features, atoms=sep_sampled, \
                                    data={'sep': sep_prepared_plus_indmom, "sf1_art": sf1_art},
                                    molecule_key='sep_sampled', split_strategy='ticker', \
                                    num_processes=num_processes, molecules_per_process=1)
    ...
    sep_tasks = [
        {  # sorted values ???
            ...
            "callback": extend_sep_for_sampling,
        # Callback to modify primary molecules individually, requires splitting according to split_strategy
        ...
        # Whether to combine and store the resulting molecules to disk (as a csv file)
            "sort_by": ["ticker", "date"],  # Sorting parameters, used both for molecules individually and when combined
            "cache_result": False,
        # Whether to cache the resulting molecules, because they are needed later in the chain
            "disk_name": "sep_extended",
        # Name of molecules saved as pickle in cache_dir or as one csv file in save_dir
        },
        {
    ...
    sep_featured_2 = pandas_chaining_mp_engine(tasks=sep_tasks, primary_atoms="sep", atoms_configs=atoms_configs, \
                                               split_strategy="ticker", num_processes=num_processes,
                                               cache_dir=cache_dir, save_dir=save_dir, sort_by=["ticker", "date"], \
    ...
    if failed == True:
        ...
        for pos in errors:
            print("Failed at position: ", pos, " Corr: ", sep_featured.iloc[pos[0]][pos[1]], "Othr: ",
...
def testing_sep_featured_fast():
    sep_featured = pd.read_csv("./testing_datasets/sep_featured.csv", parse_dates=["date"], index_col="date",
                               low_memory=False)
    sep_featured_2 = pd.read_csv("./testing_datasets/sep_featured_2.csv", parse_dates=["date"], index_col="date",
                                 low_memory=False)
...
def testing_sf1_featured():
    ...
    sf1_tasks = [
        ...
        # Whether to combine and store the resulting molecules to disk (as a csv file)
            "sort_by": ["ticker", "calendardate", "datekey"],
        # Sorting parameters, used both for molecules individually and when combined
            "cache_result": False,
        # Whether to cache the resulting molecules, because they are needed later in the chain
            "disk_name": "sf1_art_featured",
        # Name of molecules saved as pickle in cache_dir or as one csv file in save_dir
        },
        {
    ...
    sf1_featured_2 = pandas_chaining_mp_engine(tasks=sf1_tasks, primary_atoms="sf1_art",
                                               atoms_configs=sf1_atoms_configs, \
                                               split_strategy="ticker", num_processes=num_processes,
    ...
    if failed == True:
        ...
        for pos in errors:
            print("Failed at position: ", pos, " Corr: ", sf1_featured.iloc[pos[0]][pos[1]], "Othr: ", \
                  sf1_featured_2.iloc[pos[0]][pos[1]], sf1_featured.iloc[pos[0]]["datekey"],
                  sf1_featured.iloc[pos[0]]["ticker"], \
...
def testing_sf1_featured_fast():
    sf1_featured = pd.read_csv("./testing_datasets/sf1_featured.csv", parse_dates=["calendardate", "datekey"],
                               index_col="calendardate", low_memory=False)
    sf1_featured_2 = pd.read_csv("./testing_datasets/sf1_featured_2.csv", parse_dates=["calendardate", "datekey"],
                                 index_col="calendardate", low_memory=False)
