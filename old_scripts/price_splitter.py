import sys
from automated_trading_system.dataset_builder.dataset import Dataset
from automated_trading_system.logger.logger import Logger
from automated_trading_system.helpers.helpers import print_exception_info

from automated_trading_system.dataset_builder.feature_builders import book_to_market, book_value, cash_holdings
from automated_trading_system.helpers.custom_exceptions import FeatureError
import pandas as pd

if __name__ == "__main__":

    ticker_datasets = list()

    for i in range(10):
        set_nr = i + 1
        path = "./datasets/tickers/set_" + str(set_nr) + ".csv"
        try:
            ticker_dataset = Dataset(path)
        except Exception as e:
            print_exception_info(e)
            sys.exit()
        
        ticker_datasets.append(ticker_dataset)



    try:
        df = pd.read_csv("./datasets/sharadar/SHARADAR_SEP.csv")
        print(df.head())
        prices_dataset = Dataset.from_df(df)
        # prices_dataset = Dataset("./datasets/sharadar/SHARADAR_SEP.csv", None, "./datasets/sharadar/indicators_demo.csv")
    except Exception as e:
        print_exception_info(e)
        sys.exit()

    for index, ticker_dataset in enumerate(ticker_datasets):
        tickers = ticker_dataset.data["ticker"].values.tolist()
        # print(ticker_dataset.data["ticker"].values.tolist())
        prices_df = prices_dataset.data.loc[prices_dataset.data["ticker"].isin(tickers)].copy()
        prices_dataset_chunk = Dataset.from_df(prices_df)
        prices_dataset_chunk.sort(["ticker", "date"])
        # print(prices_df.head())

        set_nr = index + 1
        path = "./datasets/prices/set_" + str(set_nr) + ".csv"
        prices_dataset_chunk.to_csv(path)
    
